!function(t, a) {
    var r = 1e4
        ...
        umidPCModule: {
            stable: ["AWSC/WebUMID/1.78.0/um.js"],
            grey: ["AWSC/WebUMID/1.79.2/um.js"],
            ratio: 9999
        },
        ncPCModule: {
            stable: ["js/nc/60.js"],
    ...
    function U(t, r, s) {
        if (s)
            ...
            for (var c = 0; c < t.length; c++) {
                ...
                m && m.parentNode ? m.parentNode.insertBefore(h, m) : (m = a.body || a.head,
                m && m.appendChild(h))
            }
    }
    function I(a, r, s) {
    ...
    function E(t, a, r, s) {
        ...
        var w = r && r.cdn
            , A = r && r.sync
            , T = r && r.timeout || 5e3;
        if (0 !== p.depends.length)
            for (var y = 0; y < p.depends.length; y++) {
                var j = p.depends[y];
                r && (delete r.sync,
                    delete r.timeout,
                    delete r.cdn),
        ...
        var W = p.sync;
        A && (W = A);
        // D = //g.alicdn.com/AWSC/uab/122.js?d=27
        var D = c(p.name, w);
        U(D, "AWSC_" + p.name, W)
    }
    function P(t, a, r) {
    ...
    function O(t, a, r) {
        ...
        try {
            ...
            for (var w = void 0, k = 0; k < v.length; k++) {
                var A = v[k];
                A.module === c && A.name === a && (s = A.options && A.options.throwExceptionInCallback,
                    w = A,
                    clearTimeout(w.timeoutTimer),
                    delete w.timeoutTimer,
                    w.exportObj = r,
                    A.state === h || A.state === b ? (w.state = p,
                        x(w, s)) : void 0)
            }
            w || (w = {},
                w.module = c,
            ...
            t.AWSC.configFY = function(a, r, s, c) {
                ...
                v.umidToken = "defaultToken1_um_not_loaded@@" + location.href + "@@" + (new Date).getTime(),
                    t.AWSC.use("um", function(t, a) {
                        "loaded" === t ? (v.umidToken = "defaultToken3_init_callback_not_called@@" + location.href + "@@" + (new Date).getTime(),
                            a.init(r, function(t, a) {
                                "success" === t ? v.umidToken = a.tn : v.umidToken = "defaultToken4_init_failed with " + t + "@@" + location.href + "@@" + (new Date).getTime(),
                                    h = !0,
                                    w()
                            })) : (v.umidToken = "defaultToken2_load_failed with " + t + "@@" + location.href + "@@" + (new Date).getTime(),
                            h = !0,
                            w())
                    });
                var p = !1;
